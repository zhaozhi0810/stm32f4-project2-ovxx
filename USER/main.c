...
{ 
	...
	int dydy = 3;//i为前进磁轨条数，在第几条磁轨后掉头
	int x = 2*dydy;//x=x+a,a为回去的磁轨条数
	int y=1;
	u8 res;							 
	u8 *pname;				//带路径的文件名    
	u8 q;						 
...
//	CARKEY_Init();
	...
	delay_ms(500);
	printf("2022-05-04 ----4-正常\r\n");
	
	//jpeg_buf0=mymalloc(SRAMIN,jpeg_dma_bufsize*4);
	jpeg_buf0=mymalloc(SRAMIN,jpeg_dma_bufsize*4);
	delay_ms(500);	//为jpeg dma接收申请内存	
	//jpeg_buf1=mymalloc(SRAMIN,jpeg_dma_bufsize*4);
	jpeg_buf1=mymalloc(SRAMIN,jpeg_dma_bufsize*4);
	delay_ms(500);	//为jpeg dma接收申请内存	
	jpeg_data_buf=mymalloc(SRAMEX,300*1024);
	delay_ms(500);	//为jpeg文件申请内存(最大300KB)
 	pname=mymalloc(SRAMEX,30);
	delay_ms(500);//为带路径的文件名分配30个字节的内存	 
	...
	res = OV5640_Focus_Init();
//	printf("OV5640_Focus_Init res = %d \r\n",res);	
	OV5640_Light_Mode(0);	//自动模式
	OV5640_Color_Saturation(3);//色彩饱和度0
	OV5640_Brightness(4);	//亮度0
	OV5640_Contrast(3);		//对比度0
	OV5640_Sharpness(33);	//自动锐度
	res = OV5640_Focus_Constant();//启动持续对焦
//	printf("OV5640_Focus_Constant res = %d \r\n",res);	
	My_DCMI_Init();			//DCMI配置

	
	printf("2022-05-04 ----5-正常\r\n");
	//小车
	 
	stop();
	delay_ms(2000);
	printf("2022-05-04 ----6-正常\r\n");
...
//	My_DCMI_Init();			//DCMI配置
	...
	printf("2022-05-04 ----7-正常\r\n");
	
	
	while(1)
	{	
		if(a<dydy||(a>dydy&&a<x))//----------------------------------------------------普通拍照节点
		{
		...
		if(a==dydy||a==x)//a=i与a=x的状态，i,x为调头节点
		{
			if(KEY0==0||KEY1==0||KEY2==0||KEY3==0||KEY4==0||KEY5==0||KEY6==0||KEY7==0)
			{
				Move_forward();
